...
Starts and stops Nexus services based on the setup method configured
...
def check_frontend_running() -> bool:
    """Check if the Nexus frontend is running (not other Next.js apps like Cursor)."""
...
def check_backend_running() -> bool:
    """Check if the Nexus backend is running."""
    root_dir = str(Path.cwd())
...
def start_manual_services(compose_cmd: list) -> int:
    ...
    print(f"{Colors.CYAN}Access Nexus at: http://localhost:3000{Colors.ENDC}\n")
...
def show_help():
    ...
    print(__doc__)
    print("Commands:")
    print("  start     Start all Nexus services")
    print("  stop      Stop all Nexus services")
