...
from nexus.utils import print_stream
...
async def main():
    ...
    # Create the MCP tools client with the URL of the MCP server that's accessible by the Nexus instance
    mcp_tools = nexus.MCPTools(
        "http://localhost:4000/mcp/",  # Since we are running Nexus locally, we can use the local URL
        "Nexus",
        allowed_tools=["get_wind_direction"],
    )
    await mcp_tools.initialize()

    nexus_client = nexus.Nexus(
        os.getenv("NEXUS_API_KEY", "pk_xxx:sk_xxx"),
    ...
    if not agent_id:
        agent = await nexus_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await nexus_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await nexus_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await nexus_client.Thread.get(thread_id)
