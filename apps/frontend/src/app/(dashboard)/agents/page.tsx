...
export default function AgentsPage() {
  ...
  const [marketplaceFilter, setMarketplaceFilter] = useState<'all' | 'nexus' | 'community' | 'mine'>('all');
  ...
  const marketplaceQueryParams = useMemo(() => {
    ...
    if (marketplaceFilter === 'nexus') {
      params.is_nexus_team = true;
    } else if (marketplaceFilter === 'community') {
      params.is_nexus_team = false;
    ...
    if (marketplaceTemplates?.templates) {
      marketplaceTemplates.templates.forEach(template => {
        const item: MarketplaceTemplate = {
          ...
          is_nexus_team: template.is_nexus_team,
