...
const LoggedInMenu: React.FC<UnifiedConfigMenuProps> = memo(function LoggedInMenu({
    ...
    const nexusAgent = useMemo(() => {
        return agents.find(a => a.metadata?.is_nexus_default === true);
    }, [agents]);

    const placeholderNexusAgent = useMemo(() => ({
        agent_id: undefined,
        name: 'Nexus',
        metadata: { is_nexus_default: true }
    }), []);
    ...
    const displayAgent = useMemo(() => {
        ...
        if (nexusAgent) return nexusAgent;
        return agents[0];
    }, [agents, selectedAgentId, nexusAgent]);
    ...
    const renderAgentIcon = useCallback((agent: any, size: number = 32) => {
        if (!agent && (isLoading || nexusAgent)) {
            return <AgentAvatar isNexusDefault={true} agentName="Nexus" size={size} className="flex-shrink-0 !border-0" />;
        }
        return <AgentAvatar agent={agent} agentId={agent?.agent_id} size={size} className="flex-shrink-0 !border-0" />;
    }, [isLoading, nexusAgent]);
                    ...
                    {canLoadMore && (
                        <div className={cn("pt-2", compact ? "px-2" : "px-3")}>
                            ...
                            >
                                {isFetching ? (
                                    <>
                                        <NexusLoader size="small" className="mr-2" />
                                        Loading...
                                    </>
                                ) : (
                                    `Load more`
    ...
    // Check if current agent is Nexus (used in multiple places)
    const isNexusAgent = !displayAgent?.name || displayAgent?.name === 'Nexus';
                        ...
                        <div className="px-4 pb-2">
                            ...
                            >
                                {isNexusAgent ? (
                                    ...
                                    <div className="flex items-center justify-center w-10 h-10 bg-card border-[1.5px] border-border flex-shrink-0" style={{ borderRadius: '10.4px' }}>
                                        {renderAgentIcon(isLoading && !displayAgent ? placeholderNexusAgent : displayAgent, 40)}
                                    </div>
                                )}
                                <span className="flex-1 truncate text-base font-medium text-left min-w-0">
                                    {isNexusAgent ? 'Nexus' : displayAgent?.name}
                                </span>
    ...
    }, [mobileSection, searchQuery, onAgentSelect, displayAgent, isLoading, placeholderNexusAgent, renderAgentIcon, selectedAgentId, AgentsList, CreateWorkerButton, WorkerSettingsButtons, isNexusAgent]);
        ...
        >
            {onAgentSelect ? (
                <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                    {renderAgentIcon(isLoading && !displayAgent ? placeholderNexusAgent : displayAgent, 24)}
                            ...
                            {onAgentSelect && (
                                <>
                                    ...
                                    <div className="px-2 pb-2">
                                        <SpotlightCard className={cn("transition-colors cursor-pointer bg-transparent", isNexusAgent ? "p-2" : "")}>
                                            <DropdownMenuSub>
                                                <DropdownMenuSubTrigger className="flex items-center gap-3 text-sm cursor-pointer px-1 hover:bg-transparent focus:bg-transparent data-[state=open]:bg-transparent w-full">
                                                    <div className="flex items-center gap-3">
                                                        {renderAgentIcon(isLoading && !displayAgent ? placeholderNexusAgent : displayAgent)}
                ...
                <span className="inline-block">
                    ...
                    >
                        <div className="flex items-center gap-2 min-w-0 max-w-[180px]">
                            ...
                            <span className="truncate text-sm font-medium">Nexus</span>
                            <ChevronDown size={12} className="opacity-60 flex-shrink-0" />
                        </div>
                    </Button>
                </span>
