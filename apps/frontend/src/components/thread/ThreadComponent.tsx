...
import { useNexusComputerStore, useSetIsSidePanelOpen } from '@/stores/nexus-computer-store';
...
export function ThreadComponent({ projectId, threadId, compact = false, configuredAgentId, isShared = false, isNew = false, preCreatedThreadId }: ThreadComponentProps) {
  ...
  const storeIsNexusAgentFn = useIsNexusAgentFn();
  ...
  const isNexusAgent = isShared ? false : storeIsNexusAgentFn;
  ...
  const openFileInComputer = useNexusComputerStore((state) => state.openFileInComputer);
  const openFileBrowser = useNexusComputerStore((state) => state.openFileBrowser);
  const setSandboxContext = useNexusComputerStore((state) => state.setSandboxContext);
  ...
  const handleModeStarterAction = useCallback(async (
    ...
    // Open the side panel to show NexusComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  const handleStarterPrompt = useCallback((prompt: string, placeholderInfo?: { start: number; end: number }) => {
    ...
    // Keep the side panel open to show NexusComputer
    if (!isSidePanelOpen) {
      toggleSidePanel();
    }
  ...
  useEffect(() => {
    if (projectName) {
      document.title = `${projectName} | Nexus`;
      ...
      if (metaDescription) {
        metaDescription.setAttribute(
          'content',
          `${projectName} - Interactive Worker conversation powered by Nexus`,
        );
      }

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute('content', `${projectName} | Nexus`);
      }
