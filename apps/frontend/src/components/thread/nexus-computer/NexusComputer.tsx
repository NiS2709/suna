...
import { 
  ...
  useNexusComputerPendingToolNavIndex,
  useNexusComputerClearPendingToolNav,
} from '@/stores/nexus-computer-store';
...
const FLOATING_LAYOUT_ID = 'nexus-computer-float';

export const NexusComputer = memo(function NexusComputer({
  isOpen,
  onClose,
  toolCalls,
  currentIndex,
  ...
  const pendingToolNavIndex = useNexusComputerPendingToolNavIndex();
  const clearPendingToolNav = useNexusComputerClearPendingToolNav();
  ...
  useEffect(() => {
    ...
    if (projectChanged || sandboxChanged) {
      console.log('[NexusComputer] Project or sandbox changed, resetting local state', { projectId, sandboxId });
  ...
  if (isMobile) {
    const handleDrawerKeyDown = (e: React.KeyboardEvent) => {
      ...
      // Prevent Escape / Esc from closing the Nexus Computer.
    ...
    return (
      <Drawer
        ...
        // Never allow Esc/Escape to dismiss the Nexus Computer.
  ...
  return (
    <motion.div
      ...
      layoutId="nexus-computer-window"
      initial={{ opacity: 1 }}
      animate={{ opacity: 1 }}
      transition={{
        layout: {
