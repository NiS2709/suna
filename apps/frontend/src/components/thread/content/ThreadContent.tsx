...
// Reusable agent header - shows Nexus logo for Nexus, avatar+name for others
const AgentHeader = memo(function AgentHeader({ agentInfo }: { agentInfo: AgentInfo }) {
  if (agentInfo.name === "Nexus") {
    return (
      <img
        src="/nexus-logomark-white.svg"
        alt="Nexus"
...
const AssistantGroupRow = memo(function AssistantGroupRow({
  ...
  return (
    <div key={group.key} ref={isLastGroup ? latestMessageRef : null}>
      <div className="flex flex-col gap-2">
        {/* Reasoning section with integrated Nexus icon */}
        {reasoningSection}
  ...
  function ThreadContent({
    ...
    const agentInfo = useMemo<AgentInfo>(() => {
      ...
      if (recentAssistantWithAgent?.agents?.name) {
        ...
        const name =
          typeof rawName === "string" ? rawName : String(rawName || "Nexus");
        return {
          name,
          avatar: (
            <div className="h-5 w-5 flex items-center justify-center rounded text-xs">
      ...
      const fallbackName = typeof agentName === "string" ? agentName : "Nexus";
      return {
        name: fallbackName || "Nexus",
        avatar: agentAvatar,
      };
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="space-y-2">
                  <div className="animate-shimmer inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium text-primary bg-primary/10 rounded-md border border-primary/20">
                    ...
                    <NexusLoader size="small" className="ml-auto" />
                  </div>
                </div>
              </div>
            </div>
