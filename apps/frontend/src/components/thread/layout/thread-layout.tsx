...
import { NexusComputer, ToolCallInput } from '@/components/thread/nexus-computer';
...
export const ThreadLayout = memo(function ThreadLayout({
  ...
  // Nexus Computer Store - for handling file open requests
  const { shouldOpenPanel, clearShouldOpenPanel, openFileInComputer, openFileBrowser } = useNexusComputerStore();
  ...
  // Handle file click - now opens in Nexus Computer instead of modal
  ...
  const selectedFilePath = useNexusComputerStore((state) => state.selectedFilePath);
