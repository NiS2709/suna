...
export function AgentConfigurationDialog({
  ...
  const isNameEditable = !isViewingOldVersion && (restrictions.name_editable !== false) && !isNexusAgent;
  const isSystemPromptEditable = !isViewingOldVersion && (restrictions.system_prompt_editable !== false) && !isNexusAgent;
  const areToolsEditable = !isViewingOldVersion && (restrictions.tools_editable !== false) && !isNexusAgent;
    ...
    if (!isNameEditable) {
      if (isNexusAgent) {
        toast.error("Name cannot be edited", {
          description: "Nexus's name is managed centrally and cannot be changed.",
        });
      }
      setEditName(formData.name);
      setIsEditingName(false);
    ...
    if (!isSystemPromptEditable) {
      if (isNexusAgent) {
        toast.error("System prompt cannot be edited", {
          description: "Nexus's system prompt is managed centrally.",
        });
      }
      return;
    }
        ...
        <DialogContent className="max-w-5xl h-[85vh] overflow-hidden p-0 gap-0 flex flex-col">
                              ...
                              {agents.map((agent: any) => (
                                ...
                                >
                                  <AgentAvatar
                                    ...
                                    isNexusDefault={agent.metadata?.is_nexus_default}
                                    size={24}
                                    className="flex-shrink-0"
                                  />
                    ...
                    {isNexusAgent && (
                      <Alert className="mb-4 bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:border-blue-900">
                        ...
                        <AlertDescription className="text-sm text-blue-800 dark:text-blue-300">
                          You can't edit the main Nexus, but you can create a new AI Worker that you can modify as you wish.
                        </AlertDescription>
                      </Alert>
                    )}
          ...
          <DialogFooter className="px-6 py-4 border-t bg-background flex-shrink-0">
            ...
            >
              {isSaving ? (
                <>
                  <NexusLoader customSize={16} className="mr-1" />
                  Saving...
                </>
              ) : (
                <>
