...
const CardAvatar: React.FC<{ 
  ...
  const isNexusAgent = data.metadata?.is_nexus_default === true;
...
const AgentBadges: React.FC<{ data: BaseAgentData, isNexusAgent: boolean }> = ({ data, isNexusAgent }) => (
  <div className="flex gap-1">
    {!isNexusAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!isNexusAgent && data.is_public && (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return <MarketplaceBadge isNexusTeam={data.is_nexus_team} isOwner={isOwner} />;
        case 'template':
          return <TemplateBadge isPublic={data.is_public} />;
        case 'agent':
          return <AgentBadges data={data} isNexusAgent={isNexusAgent} />;
        default:
          return null;
      }
    };
