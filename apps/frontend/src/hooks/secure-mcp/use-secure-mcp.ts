...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_nexus_team !== undefined) searchParams.set('is_nexus_team', params.is_nexus_team.toString());
...
export function useNexusTeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'nexus-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view Nexus templates');
      }

      const response = await fetch(`${API_URL}/templates/nexus-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });
