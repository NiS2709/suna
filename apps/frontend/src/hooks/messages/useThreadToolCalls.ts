...
import { ToolCallInput } from '@/components/thread/nexus-computer';
...
import { useNexusComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/nexus-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = useNexusComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure NexusComputer switches to tools view
