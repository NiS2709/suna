...
export function useAgentStartInput(options: UseAgentStartInputOptions = {}): UseAgentStartInputReturn {
  ...
  // Determine if Nexus agent is selected (for modes panel)
  // For unauthenticated users, always show as Nexus agent (for landing page modes panel)
  const isNexusAgent = !user 
    ? true // Unauthenticated users always see Nexus modes
    : isLoadingAgents 
