...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      const powerModel = availableModels.find(m => m.id === 'nexus/power' && !m.requiresSubscription);
      if (powerModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to nexus/power');
        setSelectedModel('nexus/power');
      }
    }
