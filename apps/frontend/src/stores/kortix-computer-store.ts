...
interface NexusComputerState {
  ...
  // Clear pending tool nav after NexusComputer processes it
...
export const useNexusComputerStore = create<NexusComputerState>()(
  devtools(
    (set, get) => ({
      ...initialState,
      ...
      setSandboxContext: (sandboxId: string | null) => {
        ...
        if (currentSandboxId !== sandboxId) {
          console.log('[NexusComputerStore] Sandbox context changed:', currentSandboxId, '->', sandboxId);
          set({
      ...
      clearFileState: () => {
        console.log('[NexusComputerStore] Clearing file state');
        set({
          ...initialFileState,
        });
      },
      ...
      reset: () => {
        console.log('[NexusComputerStore] Full reset');
        set(initialState);
      },
    }),
    {
      name: 'nexus-computer-store',
    }
  )
);
...
// Sandbox context
export const useNexusComputerSandboxId = () =>
  useNexusComputerStore((state) => state.currentSandboxId);

export const useSetSandboxContext = () =>
  useNexusComputerStore((state) => state.setSandboxContext);

// Main view state
export const useNexusComputerActiveView = () => 
  useNexusComputerStore((state) => state.activeView);

// Individual selectors for files state (stable, primitive values)
export const useNexusComputerFilesSubView = () =>
  useNexusComputerStore((state) => state.filesSubView);

export const useNexusComputerCurrentPath = () =>
  useNexusComputerStore((state) => state.currentPath);

export const useNexusComputerSelectedFilePath = () =>
  useNexusComputerStore((state) => state.selectedFilePath);

export const useNexusComputerFilePathList = () =>
  useNexusComputerStore((state) => state.filePathList);

export const useNexusComputerCurrentFileIndex = () =>
  useNexusComputerStore((state) => state.currentFileIndex);

// Legacy combined selector (for backward compatibility) - use individual selectors in components
export const useNexusComputerFilesState = () => ({
  filesSubView: useNexusComputerStore((state) => state.filesSubView),
  currentPath: useNexusComputerStore((state) => state.currentPath),
  selectedFilePath: useNexusComputerStore((state) => state.selectedFilePath),
  filePathList: useNexusComputerStore((state) => state.filePathList),
  currentFileIndex: useNexusComputerStore((state) => state.currentFileIndex),
});

// Actions are stable references (functions don't change)
export const useNexusComputerActions = () =>
  useNexusComputerStore((state) => ({
...
export const useNexusComputerPendingToolNavIndex = () =>
  useNexusComputerStore((state) => state.pendingToolNavIndex);

export const useNexusComputerClearPendingToolNav = () =>
  useNexusComputerStore((state) => state.clearPendingToolNav);

// Side panel state selectors
export const useIsSidePanelOpen = () =>
  useNexusComputerStore((state) => state.isSidePanelOpen);

export const useSetIsSidePanelOpen = () =>
  useNexusComputerStore((state) => state.setIsSidePanelOpen);
