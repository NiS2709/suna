...
export function AgentProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    if (agents.length > 0 && !hasInitialized && !isLoading) {
      const autoSelectDefaultAgent = () => {
        ...
        const nexusAgent = agents.find(agent => agent.metadata?.is_nexus_default);
        const defaultAgent = nexusAgent || agents[0];
  ...
  const getDefaultAgent = React.useCallback((): Agent | null => {
    const nexusAgent = agents.find(agent => agent.metadata?.is_nexus_default);
    return nexusAgent || agents[0] || null;
  }, [agents]);
  ...
  const isNexusAgent = React.useCallback((): boolean => {
    const currentAgent = getCurrentAgent();
    return currentAgent?.metadata?.is_nexus_default || false;
  }, [getCurrentAgent]);
