...
import NexusSymbolIcon from '@/assets/brand/nexus-symbol.svg';
...
/**
 * Check if a model ID corresponds to a Nexus mode (Basic or Advanced)
 */
export function isNexusMode(modelId: string): boolean {
  // New Nexus registry IDs
  if (modelId === 'nexus/basic' || modelId === 'nexus/power' ||
      modelId === 'nexus-basic' || modelId === 'nexus-power') {
    return true;
  }
...
export function getModelProvider(modelId: string): ModelProvider {
  ...
  if (parts.length > 1) {
    ...
    if (['openai', 'anthropic', 'google', 'xai', 'moonshotai', 'bedrock', 'openrouter', 'nexus'].includes(provider)) {
...
export function getModelProviderIcon(modelId: string): React.FC<SvgProps> {
  ...
  const iconMap: Record<ModelProvider, React.FC<SvgProps>> = {
    nexus: NexusSymbolIcon, // Nexus modes use the Nexus symbol
