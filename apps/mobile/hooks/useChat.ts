...
import { useNexusComputerStore } from '@/stores/nexus-computer-store';
import { 
...
export function useChat(): UseChatReturn {
  ...
  // Prioritizes nexus/basic, then nexus/power, then first accessible model
  const getDefaultModelId = useCallback((models: typeof accessibleModels): string | undefined => {
    // nexus/basic should be first for free users since power is not accessible
    const basicModel = models.find(m => m.id === 'nexus/basic');
    if (basicModel) return basicModel.id;

    const powerModel = models.find(m => m.id === 'nexus/power');
  ...
  const VALID_MODEL_IDS = useMemo(() => new Set(['nexus/basic', 'nexus/power']), []);
  ...
  useEffect(() => {
    ...
    // CRITICAL: Only allow nexus/basic or nexus/power models
    // Other models (like nexus/kimi-k2.5) are internal and should not be used
  ...
  const currentModel = useMemo(() => {
    ...
    // CRITICAL: Only use nexus/basic or nexus/power
    // Never use other models like nexus/kimi-k2.5 (internal/legacy)
    ...
    // Fallback: prioritize nexus/basic, then nexus/power
    const basicModel = accessibleModels.find(m => m.id === 'nexus/basic');
    if (basicModel) return basicModel.id;

    const powerModel = accessibleModels.find(m => m.id === 'nexus/power');
    if (powerModel) return powerModel.id;
  ...
  const loadThread = useCallback((threadId: string) => {
    ...
    // Reset Nexus Computer state when switching threads
    useNexusComputerStore.getState().reset();
    log.log('[useChat] Reset Nexus Computer state');
  ...
  const startNewChat = useCallback(() => {
    ...
    // Reset Nexus Computer state when starting new chat
    useNexusComputerStore.getState().reset();
    log.log('[useChat] Reset Nexus Computer state for new chat');
