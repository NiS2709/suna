...
export function useAuth() {
   ...
   * Uses nexus:// deep link - works when app is installed
   */
  const signInWithMagicLink = useCallback(async ({ email, acceptedTerms }: { email: string; acceptedTerms?: boolean }) => {
    try {
      ...
      const emailRedirectTo = `nexus://auth/callback${params.toString() ? `?${params.toString()}` : ''}`;
      ...
      if (magicLinkError) {
        log.error('❌ Supabase rejected redirect URL:', {
          ...
          hint: 'Make sure nexus://auth/callback is in Supabase Dashboard → Auth → Redirect URLs',
        });
      }
