...
interface AvatarProps extends ViewProps {
  ...
  /** Use Nexus symbol instead of icon (for NEXUS agent) */
  useNexusSymbol?: boolean;
 ...
 * - Nexus symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Nexus symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Nexus symbol always uses solid black bg with white icon
    if (useNexusSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useNexusSymbol ? 0 : (showBorder ? 1.5 : 0),
