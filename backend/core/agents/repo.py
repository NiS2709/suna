...
async def get_default_agent_id(account_id: str) -> Optional[str]:
    ...
    WHERE account_id = :account_id 
      AND metadata->>'is_nexus_default' = 'true'
    LIMIT 1
    """
    result = await execute_one(sql, {"account_id": account_id})
...
async def get_shared_nexus_agent(admin_user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    if admin_user_id:
        sql = """
    ...
    sql = """
    SELECT agent_id, account_id FROM agents 
    WHERE metadata->>'is_nexus_default' = 'true'
    LIMIT 1
    """
    result = await execute_one(sql)
    return dict(result) if result else None
