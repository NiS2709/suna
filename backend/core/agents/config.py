        ...
        if agent_type == "nexus":
            ...
            if static_config:
                ...
                agent_config = {
                    ...
                    'is_nexus_default': static_config['is_nexus_default'],
                ...
                logger.info(f"⏱️ [AGENT CONFIG FAST] Nexus (cached type): {(time.time() - t) * 1000:.1f}ms")
                return agent_config
    ...
    try:
        if not agent_id:
            ...
            if is_new_thread:
                ...
                await ensure_nexus_installed(account_id)
            ...
            if default_agent_id:
                ...
                logger.warning(f"[AGENT LOAD] No default agent found for account {account_id}, searching for shared Nexus")
                agent_data = await _find_shared_nexus_agent()
...
async def _find_shared_nexus_agent():
    from core.agents.agent_loader import get_agent_loader
    ...
    shared_agent = await agents_repo.get_shared_nexus_agent(admin_user_id)
    ...
    if shared_agent:
        ...
        if admin_user_id and shared_agent['account_id'] == admin_user_id:
            logger.info(f"✅ Using system Nexus agent from admin user: {agent_data.name} ({agent_data.agent_id})")
        else:
            logger.info(f"Using shared Nexus agent: {agent_data.name} ({agent_data.agent_id})")
        return agent_data
    
    if admin_user_id:
        logger.warning(f"⚠️ SYSTEM_ADMIN_USER_ID configured but no Nexus agent found for user {admin_user_id}")
    
    logger.error("❌ No Nexus agent found! Set SYSTEM_ADMIN_USER_ID in .env")
    return None
