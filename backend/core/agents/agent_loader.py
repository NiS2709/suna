    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.config_loaded:
            result.update({
                ...
                "is_nexus_default": self.is_nexus_default,
...
class AgentLoader:
    ...
    async def load_agent(
        ...
        if load_config and agent_row.get('current_version_id'):
            ...
            await set_cached_agent_config(
                ...
                is_nexus_default=agent_data.is_nexus_default
            )
    ...
    def _dict_to_agent_data(self, data: Dict[str, Any]) -> AgentData:
        ...
        return AgentData(
            ...
            is_nexus_default=data.get('is_nexus_default', False),
    ...
    def _row_to_agent_data(self, row: Dict[str, Any]) -> AgentData:
        ...
        For Nexus agents, always overrides name and description from NEXUS_CONFIG
        regardless of what's stored in the database.
        """
        metadata = row.get('metadata', {}) or {}
        is_nexus_default = metadata.get('is_nexus_default', False)
        
        # For Nexus agents, always use name from NEXUS_CONFIG (never DB value)
        if is_nexus_default:
            from core.config.nexus_config import NEXUS_CONFIG
            name = NEXUS_CONFIG['name']
            description = NEXUS_CONFIG.get('description')
        else:
            name = row['name']
        ...
        return AgentData(
            ...
            metadata=metadata,
            is_nexus_default=is_nexus_default,
            config_loaded=False
        )
    
    async def _load_agent_config(self, agent: AgentData, user_id: str):
        """Load full configuration for a single agent."""
        if agent.is_nexus_default:
            await self._load_nexus_config(agent, user_id)
        else:
            await self._load_custom_config(agent, user_id)
        
        agent.config_loaded = True
    
    async def _load_nexus_config(self, agent: AgentData, user_id: Optional[str] = None):
        """
        Load Nexus config using static in-memory config + cached user MCPs.
        
        Static parts (prompt, model, tools) = instant from memory
        User MCPs = check cache first, then DB if miss
        Always overrides name from NEXUS_CONFIG regardless of DB value.
        """
        import time
        t_start = time.time()
        
        # 1. Load static config from memory (instant, no DB)
        from core.cache.runtime_cache import get_static_nexus_config, load_static_nexus_config
        from core.config.nexus_config import NEXUS_CONFIG
        
        static_config = get_static_nexus_config()
        if not static_config:
            static_config = load_static_nexus_config()
        
        # Always override name from NEXUS_CONFIG (never use DB value)
        agent.name = NEXUS_CONFIG['name']
        agent.description = NEXUS_CONFIG.get('description')
        ...
        if agent.current_version_id and user_id:
            ...
            if cached_mcps:
                ...
                logger.debug(f"⚡ Nexus config loaded in {(time.time() - t_start)*1000:.1f}ms (MCPs from cache)")
                return
            ...
            try:
                ...
                logger.debug(f"Nexus config loaded in {(time.time() - t_start)*1000:.1f}ms (MCPs from DB, now cached)")
            except Exception as e:
                logger.warning(f"Failed to load MCPs for Nexus agent {agent.agent_id}: {e}")
                agent.configured_mcps = []
                agent.custom_mcps = []
                agent.triggers = []
        else:
            ...
            agent.custom_mcps = []
            agent.triggers = []
            logger.debug(f"⚡ Nexus config loaded in {(time.time() - t_start)*1000:.1f}ms (no MCPs)")
    ...
    async def _batch_load_configs(self, agents: list[AgentData]):
        ...
        # Get all version IDs for non-Nexus agents
        version_ids = [a.current_version_id for a in agents if a.current_version_id and not a.is_nexus_default]
        
        if not version_ids:
            # Only Nexus agents, load their configs
            for agent in agents:
                if agent.is_nexus_default:
                    await self._load_nexus_config(agent, agent.account_id)
                    agent.config_loaded = True
            return
        ...
        try:
            ...
            version_map = {}
            for agent in agents:
                if agent.current_version_id and not agent.is_nexus_default:
                    try:
                        version = await version_service.get_version(
                            agent_id=agent.agent_id,
                            version_id=agent.current_version_id,
            ...
            # Fallback: load Nexus configs only
            for agent in agents:
                if agent.is_nexus_default:
                    await self._load_nexus_config(agent, agent.account_id)
                    agent.config_loaded = True
