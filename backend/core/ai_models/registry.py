...
FREE_MODEL_ID = "nexus/minimax"
PREMIUM_MODEL_ID = "nexus/power"
IMAGE_MODEL_ID = "nexus/haiku"
...
class ModelFactory:
    ...
    def create_basic_model(main_llm: str, custom_model: Optional[str] = None) -> Model:
        ...
        if main_llm == "kimi":
            return Model(
                ...
                provider=ModelProvider.OPENROUTER,
                aliases=["nexus-basic", "Nexus Basic"],
    ...
    def create_power_model(main_llm: str, custom_model: Optional[str] = None) -> Model:
        ...
        if main_llm == "kimi":
            return Model(
                id="nexus/power",
                name="Nexus Advanced Mode",
                litellm_model_id=custom_model or default_models["kimi"],
                provider=ModelProvider.OPENROUTER,
                aliases=["nexus-power", "Nexus POWER Mode", "Nexus Power", "Nexus Advanced Mode"],
                context_window=262_144,
                capabilities=[
                    ModelCapability.CHAT,
                    ModelCapability.FUNCTION_CALLING,
