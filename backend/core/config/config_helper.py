...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for Nexus vs custom agents."""
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_nexus_default={is_nexus_default}")
    ...
    # Handle Nexus agents with special logic
    if is_nexus_default:
        return _extract_nexus_agent_config(agent_data, version_data)
    
    # Handle custom agents with versioning
    return _extract_custom_agent_config(agent_data, version_data)


def _extract_nexus_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for Nexus agents - always use central config with user customizations.
    
    Uses cached static config from runtime_cache instead of reading NEXUS_CONFIG directly.
    Always overrides name from NEXUS_CONFIG regardless of what's in the database.
    """
    ...
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using Nexus central config for agent {agent_id}")
    ...
    # Start with cached static config, but always override name from NEXUS_CONFIG
    config = {
        'agent_id': agent_data['agent_id'],
        'name': NEXUS_CONFIG['name'],  # Always override name from NEXUS_CONFIG
...
def build_unified_config(
    ...
    if nexus_metadata:
        config['nexus_metadata'] = nexus_metadata
