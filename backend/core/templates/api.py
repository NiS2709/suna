...
from .template_service import (
    ...
    NexusDefaultAgentTemplateError
)
...
class TemplateResponse(BaseModel):
    ...
    categories: List[str]
    is_public: bool
    is_nexus_team: Optional[bool] = False
...
async def create_template_from_agent(
    ...
    except NexusDefaultAgentTemplateError as e:
        logger.warning(f"Template creation failed - Nexus default agent: {e}")
...
@router.get("/nexus-all", response_model=MarketplaceTemplatesResponse)
async def get_all_nexus_templates(
    ...
    try:
        ...
        try:
            ...
            error_str = f"Error of type {type(e).__name__}"
        logger.error(f"Error getting all Nexus templates: {error_str}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_marketplace_templates(
    ...
    is_nexus_team: Optional[bool] = Query(None, description="Filter for Nexus team templates"),
